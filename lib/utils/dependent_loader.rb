...
#
# There are certain lists without which the app will be rendered useless.
#
...
class DependentLoader
  ...
  def self.start(table, &block)
    if ActiveRecord::Base.connection.tables.include?(table.to_s) && !defined?(::Rake) && !Rails.env.test?
